# Hobart House Painters — rank-and-rent microsite
...
| Item | Value |
|---|---|
| Domain | `hobarthousepainters.com.au` (registered, GoDaddy fraud-pending) |
| Live URL (interim) | https://hobarthousepainters.netlify.app |
...
│   ├── officer.astro         # /kingston/
│   ├── berwick.astro         # /new-town/
│   ├── beaconsfield.astro    # /sandy-bay/
│   ├── cardinia.astro        # /hobart-region/ (shire-wide overview)
...
├── whisper.js                # "Pete's Lead from Hobart House Painters" played to contractor before patching caller through
...
| `TENANT_BRAND` | `Hobart House Painters` (default) | What plays after the label — full whisper is `"Pete's Lead. Pete's Lead from Hobart House Painters."` |
...
3. Submit to Google Search Console (`sc-domain:hobarthousepainters.com.au`).
4. Set up Google Business Profile (needs registered business name "Hobart House Painters" first).
